...
def test_combine_multichoice_chunks() -> None:
    chunks = zip_choices(new_chunks(), new_chunks(), append_text='!', finish_reason='tool_calls')
    completion = new_chat_completion()
    combined = combine_chunks(chunks)
    assert combined.id == completion.id
    assert combined.created == completion.created
    ...
    assert combined.choices[1].message.content == '!No!,! I! am! not! sentient!.! I! am! a! computer! program! designed! to! assist! with! tasks! and! provide! information!.!'
